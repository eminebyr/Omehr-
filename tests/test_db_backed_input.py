...
OMEHR_INPUT_SOURCE=db bayrağıyla devreye giren yeni mimarinin (şema,
...
def db_root(tmp_path, monkeypatch):
    ...
    OMEHR_INPUT_SOURCE=db ile çalışacak modülleri yeniden yükler."""
    root = tmp_path
    (root / "input").mkdir()
    (root / "data").mkdir()
    (root / "output").mkdir()
...
def test_db_backed_load_produces_identical_kpis_to_excel(db_root, monkeypatch):
    ...
    # DB modu (fixture zaten OMEHR_INPUT_SOURCE=db kurdu)
    import importlib
    ...
    root, _ = db_root
    monkeypatch.setenv("OMEHR_INPUT_SOURCE", "excel")
    (root / "input" / "OMEHR_AI_NORM_TRANSFER_INPUT.xlsx").write_bytes(
        (
            __import__("pathlib").Path(__file__).resolve().parents[1]
            / "ORNEK_TEST_VERISI" / "OMEHR_AI_NORM_TRANSFER_INPUT.xlsx"
        ).read_bytes()
...
def test_excel_mode_unaffected_when_flag_not_set(monkeypatch, tmp_path):
    """OMEHR_INPUT_SOURCE ayarlanmamışsa (varsayılan), sistem HİÇBİR
...
def test_excel_read_shim_only_intercepts_the_input_file(db_root):
    """Shim, YALNIZ OMEHR_AI_NORM_TRANSFER_INPUT.xlsx adlı dosyayı
    ...
    assert _girdi_dosyasi_mi("input/OMEHR_AI_NORM_TRANSFER_INPUT.xlsx") is True
