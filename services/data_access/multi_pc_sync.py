...
def _sayfa_hash(input_path: Path, sheet_name: str) -> str | None:
    try:
        from services.data_access.cached_excel_reader import read_sheet_cached
...
def invalidate_local_reports_if_shared_input_changed(root: Path, input_path: Path) -> bool:
    ...
    if changed:
        from services.data_access.master_data_admin import _invalidate_generated_reports
