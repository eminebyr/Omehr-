...
def _send_smtp(subject: str, body: str, recipients: list[str], files: list[Path]) -> str:
    host = os.getenv("OMEHR_SMTP_HOST", "").strip()
    sender = os.getenv("OMEHR_SMTP_FROM", "").strip()
    ...
    port = int(os.getenv("OMEHR_SMTP_PORT", "587"))
    with smtplib.SMTP(host, port, timeout=30) as server:
        if os.getenv("OMEHR_SMTP_TLS", "1") == "1":
            server.starttls()
        username = os.getenv("OMEHR_SMTP_USER", "").strip()
        password = os.getenv("OMEHR_SMTP_PASSWORD", "")
        if username:
...
def send_outlook(subject: str, body: str, recipients: list[str], attachments=None) -> str:
    ...
    if os.getenv("OMEHR_MAIL_DRY_RUN", "0") == "1":
