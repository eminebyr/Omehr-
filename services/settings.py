...
tek kaynak burasıdır. Dosya adını değiştirmeniz gerekirse:
  ...
  2) Kod değiştirmeden BASDAS_INPUT_FILE ortam değişkenini tanımlayın.
...
DEFAULT_INPUT_FILE_NAME = "BASDAS_AI_NORM_TRANSFER_INPUT.xlsx"


def input_file_name() -> str:
    """Ana input Excel dosyasının adı.

    BASDAS_INPUT_FILE ortam değişkeni tanımlıysa onu kullanır; aksi halde
    DEFAULT_INPUT_FILE_NAME döner. Böylece dosya adı kod değiştirmeden de
    (örn. .env üzerinden) geçersiz kılınabilir.
    """
    value = os.getenv("BASDAS_INPUT_FILE", "").strip()
