...
Öncelik sırası:
  ...
  3) Hiçbiri yoksa varsayılan tek-kiracı kodu: 'OMEHR'.
...
def _oturumdan_kiraci() -> str | None:
    ...
    try:
        if not hasattr(st, "session_state"):
            return None
        kullanici = st.session_state.get("omehr_kullanici")
...
def set_session_tenant(tenant_id: str) -> None:
    ...
    import streamlit as st
    if "omehr_kullanici" not in st.session_state or not isinstance(st.session_state.get("omehr_kullanici"), dict):
        st.session_state["omehr_kullanici"] = {}
    st.session_state["omehr_kullanici"]["tenant_id"] = str(tenant_id).strip().upper()
